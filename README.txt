...
1. on steamdeck open terminal - click STEAM button, scroll down to Power, choose Switch to desktop
  menu - konsole
use the command 'passwd' to setup a password for user deck. pick something simple
...
5. use a computer to 'ssh deck@<IP address>' with password. 
run "sudo su -" to become root. 
verify with "pwd" you are under /root

6. onto the rke2 part. download the rke2 installer and save it as a file:
...
8. before running the installer, change the hostname if you plan to create a cluster
     nano /etc/hostname # set steamdeck1 or something else instead of steamdeck
   also run
...
9. setup your path. run
   ...
   add that line to the very end of .bashrc file (nano)
...
11. start it with "systemctl start rke2-server". 
